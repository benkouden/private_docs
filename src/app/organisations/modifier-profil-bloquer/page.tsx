...
import Debloquer from '@/components/debloquer'
import Del_Organisation from '@/components/delete_organisation2'
import Update_Sucess from '@/components/update_sucess'
import Sidebar from '@/components/sidebar'

const Modi_Profil_org2 = () => {
  ...
  const [isDebloquerModalOpen, setIsDebloquerModalOpen] = useState(false)
  const openDebloquerModal = () => setIsDebloquerModalOpen(true) // Ouvrir le modal
  const closeDebloquerModal = () => setIsDebloquerModalOpen(false) // Fermer le modal
    ...
    <div className='flex w-full min-h-screen text-text font-Urbanist overflow-hidden'>
      ...
      <main className='flex-1 ml-5 p-8 overflow-auto'>
        ...
        <div className=' border border-gray  mt-10 pb-[400px]'>
          <div className='h-[88px]  pr-10 flex bg-blue_Gray items-center justify-end space-x-96'>
            <span className='mr-[100px] text-text'>Pentatonic</span>
            <button
              type='submit'
          ...
          <div className='flex space-x-8 mt-8 ml-8'>
            ...
            <button
              onClick={openDebloquerModal} // Ouvrir le modal
      ...
      {isModalOpen && (
        <div className='fixed inset-0 flex items-center justify-center bg-bgmodal z-50'>
          <Del_Organisation onClose={closeModal} />
        </div>
      )}

      {/* Modal Debloquer  */}
      {isDebloquerModalOpen && (
        <div className='fixed inset-0 flex items-center justify-center bg-bgmodal z-50'>
          <Debloquer onClose={closeDebloquerModal} />
        </div>
      )}
...
export default Modi_Profil_org2
